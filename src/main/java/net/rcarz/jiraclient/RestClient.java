...
public class RestClient {
    ...
    private JSON request(HttpEntityEnclosingRequestBase req, InputStreamBody body) throws RestException, IOException {
        req.setHeader("X-Atlassian-Token", "nocheck");
        MultipartEntity ent = new MultipartEntity(HttpMultipartMode.BROWSER_COMPATIBLE, null, StandardCharsets.UTF_8);
        ent.addPart("file", body);
        req.setEntity(ent);
        return request(req);
    }
    ...
    public JSON post(String path, String filename, InputStream in) throws RestException, IOException, URISyntaxException {
        return request(new HttpPost(buildURI(path)), new InputStreamBody(in, filename));
    }
